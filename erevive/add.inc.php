...
if (isset($_POST['submit'])) {
    ...
    if (isset($_FILES['image']) && !empty($filename)) {
        ...
        if ($uploadstatus == 0) {
            ...
            header('location: editadd.php?error=upload&title=' . $name);
            $con->close();
            exit();
        }
        // All checks are successful (upload status = 1)
            ...
            if (empty($name) || empty($category) || empty($brand) || empty($age) || empty($price)) {
                // Sends user back to partially completed form
                header('location: editadd.php?error=emptyfields&name=' . $name);
                $con->close();
                exit();
            }
            // Checks characters entered into model field
                ...
                // Sends user back to partially completed form
                header('location: editadd.php?error=invalidname&name=' . $name);
                $con->close();
                exit();
            }
            else if (is_nan($price)|| $price > 99999999.99) {
                // Sends user back to partially completed form
                header('location: editadd.php?error=invalidprice&name=' . $name);
                $con->close();
                exit();
            }
            // All checks are successful
                ...
                $sql = "INSERT INTO products (name, brand, category, age, price, image, user_id) VALUES (?, ?, ?, ?, ?, ?, ?);";
                // Initialise statement
                $stmt = mysqli_stmt_init($con);
                // If statement can't be prepared, send user an error
                if (!mysqli_stmt_prepare($stmt, $sql)) {
                    header('location: editadd.php?error=sqlerror');
                    $stmt->close();
                    exit();
                }
                // If statement can be prepared
                    ...
                    mysqli_stmt_bind_param($stmt, "ssssdsi", $name, $brand, $category, $age, $price, $filename, $_SESSION['user_id']);
                    // Execute prepared statement
                   
                mysqli_stmt_execute($stmt);
                    // Redirect user with success message
                    header('location: manageitems.php?success=itemadded');
                    $stmt->close();
                    exit();
                }
            }
    ...
    // This request did not include a file, so do the original checks on title/description/genre
    else {
        // Checks for empty fields
        if (empty($name) || empty($category) || empty($brand) || empty($age) || empty($price)) {
            // Sends user back to partially completed form
        ...
        // All checks are successful
            ...
            // Define the prepared insert statement
            $sql = "INSERT INTO products (name, brand, category, age, price, user_id) VALUES (?, ?, ?, ?, ?, ?);";
            // Initialise statement
            $stmt = mysqli_stmt_init($con);
            //If statement can't be prepared, send user an error
            if (!mysqli_stmt_prepare($stmt, $sql)) {
            ...
            // If statement can be prepared
                ...
                // Bind params to prepared statment
                mysqli_stmt_bind_param($stmt, "ssssdi", $name, $brand, $category, $age, $price, $_SESSION['user_id']);
                // Execute prepared statement
                mysqli_stmt_execute($stmt);
...
// Add book form hasn't been submitted 
else {
